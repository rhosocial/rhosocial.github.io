...
window.I18N['zh-cn'] = {
  ...
  body: [
    ...
    { tag: "lead", html: "在设计和实现 rhosocial ActiveRecord 时，我们面临了一个关键决策：模型类的基类应该是什么？传统的 ORM 方案倾向于使用元类（metaclass）或装饰器来声明字段，但 ActiveRecord 选择了直接继承 Pydantic 的 <code>BaseModel</code>。这篇文章将解释这一决策背后的考量，以及由此带来的能力和约束。" },
    ...
    { tag: "h2", html: "定义字段：属性即数据表字段" },
    { tag: "p", html: "在 AR 中，模型的每个类型注解属性都对应一个数据库表字段。由于 <code>ActiveRecord</code> 直接继承 Pydantic 的 <code>BaseModel</code>，所有字段都是标准的 Pydantic 字段——Pydantic 会自动跟踪这些字段的类型、默认值和验证规则：" },
    { tag: "code", text: "from rhosocial.activerecord import ActiveRecord\n\nclass User(ActiveRecord):\n    id:    int | None = None\n    name:  str\n    email: str\n    age:   int = 0" },
    { tag: "p", html: "这里 <code>id</code>、<code>name</code>、<code>email</code>、<code>age</code> 既是 Python 类型注解，也是数据表字段。Pydantic 的 <code>BaseModel</code> 会自动记录这些字段的元信息：类型、默认值、是否必填等。AR 在此基础上，将这些字段一一映射到数据库表的列。" },
    { tag: "p", html: "这意味着开发者不需要学习两套字段声明语法——Pydantic 的语法就是 AR 的声明语法，反之亦然。这也带来了类型安全、验证管道和序列化的开箱即用体验。" },
    ...
    { tag: "h2", html: "如何避开 BaseModel 跟踪" },
    { tag: "p", html: "继承 Pydantic 的 <code>BaseModel</code> 意味着所有带有类型注解的属性都会被 Pydantic 跟踪。但在实际的 ORM 使用中，有些属性<strong>不应该</strong>被 Pydantic 跟踪或映射到数据库字段：" },
    { tag: "ul", items: [
      "<strong>关联关系</strong>：<code>posts: list[Post]</code> 这种关系属性不是数据库字段，而是通过外键查询得出的逻辑关系",
      "<strong>字段代理</strong>：有些属性在 Pydantic 层面是普通字段，但在 ORM 层面需要特殊行为（如延迟加载、计算属性）",
      "<strong>非持久化属性</strong>：一些运行时状态（如缓存、临时计算值）不应该被序列化或持久化"
    ]},
    { tag: "p", html: "AR 提供以下几种方式来避开或扩展 BaseModel 的默认跟踪行为：" },

    { tag: "h3", html: "方式一：Pydantic 的 PrivateAttr" },
    { tag: "p", html: "Pydantic v2 提供了 <code>PrivateAttr</code>，带此标记的属性不会被 Pydantic 序列化、验证或纳入 schema 中。AR 也不会将这些属性映射为数据库字段：" },
    { tag: "code", text: "from pydantic import PrivateAttr\n\nclass User(ActiveRecord):\n    name: str\n    _cache: dict = PrivateAttr(default_factory=dict)" },

    { tag: "h3", html: "方式二：字段级排除" },
    { tag: "p", html: "AR 允许在字段声明时指定数据库列映射行为。当 <code>column=None</code> 时，该字段虽然是 Pydantic 字段（参与验证和序列化），但不会被映射到数据库表列：" },
    { tag: "code", text: "class User(ActiveRecord):\n    name: str\n    computed_score: float = Field(default=0.0, column=None)  # Pydantic 管理，但非数据库字段" },

    { tag: "h3", html: "方式三：字段代理" },
    { tag: "p", html: "字段代理（Field Proxy）是 AR 的核心扩展机制。它允许在不修改 Pydantic 字段行为的前提下，在 ORM 层面叠加自定义行为——比如延迟加载、变更追踪、访问拦截等。这部分将在后续文章中详细讨论。" },
    ...
    { tag: "hr" },
    { tag: "next", html: "下一篇：流式 API——ActiveRecord.save 与 ActiveQuery，让代码读起来像英语。" }
  ]
};